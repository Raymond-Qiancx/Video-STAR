...
def get_video_path(x_path, pose_estimation, person_detection, original_video_path="/data/oss_bucket_0/yzl/Recognition/mmaction2/data/ucf101/UCF-101"):
    """根据工具决策选择对应的视频路径"""
    
    if pose_estimation == 'yes' and person_detection == 'no':
        # 只有姿态估计
        base_path = "/data/oss_bucket_0/yzl/Recognition/mmaction2/data/ucf101/UCF-101_po"
        return os.path.join(base_path, x_path)
    elif pose_estimation == 'yes' and person_detection == 'yes':
        # 姿态估计 + 人员检测
        base_path = "/data/oss_bucket_0/yzl/Recognition/mmaction2/data/ucf101/UCF-101_pose"
        return os.path.join(base_path, x_path)
    elif pose_estimation == 'no' and person_detection == 'yes':
        # 只有人员检测
        base_path = "/data/oss_bucket_0/yzl/Recognition/mmaction2/data/ucf101/UCF-101_bound"
        return os.path.join(base_path, x_path)
    else:
        # 都不需要，使用原始视频
...
def main(script_args, training_args, model_args):
    ...
    # 定义动作解释文件路径
    json_file_path = "/data/oss_bucket_0/yzl/Recognition/AFile/json/ucf101/train_rephrased.json"
            ...
            "content": [
                ...
                },
                {
                    ...
                    "text": SECOND_TURN_TEMPLATE.format(
                        ...
                        ANNOTATED_VIDEO_PATH=os.path.join("/data/oss_bucket_0/yzl/Recognition/mmaction2/data/ucf101/UCF-101_pose", example['path'])
                    )
                }
            ]
        }]
