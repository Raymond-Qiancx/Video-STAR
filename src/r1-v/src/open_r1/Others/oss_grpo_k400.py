...
def get_video_path(x_path, pose_estimation, person_detection, original_video_path="/data/oss_bucket_0/yzl/Recognition/mmaction2/data/kinetics400/videos_train_2/compress/val"):
    """根据工具决策选择对应的视频路径"""
    
    if pose_estimation == 'yes' and person_detection == 'no':
        # 只有姿态估计
        base_path = "/data/oss_bucket_0/yzl/Recognition/mmaction2/data/kinetics400/videos_train_2/compress/val_po"
        return os.path.join(base_path, x_path)
    elif pose_estimation == 'yes' and person_detection == 'yes':
        # 姿态估计 + 人员检测
        base_path = "/data/oss_bucket_0/yzl/Recognition/mmaction2/data/kinetics400/videos_train_2/compress/val_pose"
        return os.path.join(base_path, x_path)
    elif pose_estimation == 'no' and person_detection == 'yes':
        # 只有人员检测
        base_path = "/data/oss_bucket_0/yzl/Recognition/mmaction2/data/kinetics400/videos_train_2/compress/val_bound"
        return os.path.join(base_path, x_path)
    else:
        # 都不需要，使用原始视频
...
def main(script_args, training_args, model_args):
    ...
    # 定义动作解释文件路径
    json_file_path = "/data/oss_bucket_0/yzl/Recognition/AFile/json/kinetics400/train_rephrased.json"
    ...
    **Stage 2: Movement Pattern Refinement**
    ...
    def make_conversation_image_and_video(example):
        ...
        # 第二轮对话：动作识别（需要根据第一轮结果选择视频）
        # 这里我们使用原始视频，实际的工具选择会在训练过程中动态决定
        second_turn_prompt = [{
            "role": "user", 
            "content": [
                {
                ...
                },
                {
                    ...
                    "text": SECOND_TURN_TEMPLATE.format(
                        ALLOWED_ANSWERS=get_allowed_answers_str(actions, True),  # 默认不需要名词解释
                        ANNOTATED_VIDEO_PATH=os.path.join("/data/oss_bucket_0/yzl/Recognition/mmaction2/data/kinetics400/videos_train_2/compress/val_pose", example['path'])
                    )
                }
            ]
        }]
