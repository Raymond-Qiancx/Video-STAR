...
def extract_tool_decisions(first_turn_output: str) -> Dict[str, str]:
    ...
    video_exp_match = re.search(video_exp_pattern, first_turn_output, re.IGNORECASE)
    if video_exp_match:
...
def prepare_dataset(example: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    # 读取视频解释 JSON 文件，构建 problem_id -> analysis 映射
    ...
    try:
        with open(video_explanation_json_path, 'r', encoding='utf-8') as f:
            ...
            for item in video_explanation_data:
                video_explanation_dict[item["problem_id"]] = item["analysis"]
    ...
    **Stage 2: Movement Pattern Refinement**
    ...
    video_explanation = tool_decisions.get('video_explanation', 'no')
    ...
    # 根据是否需要视频解释选择文本
    need_video_explanation = video_explanation == 'yes'
    # 从 video_explanation_dict 中根据 problem_id 查找对应的 analysis
    video_analysis = video_explanation_dict.get(example.get('problem_id'), '')
            ...
            "content": [
                {
                    ...
                    example['data_type']: os.path.join("/mnt/xmap_nas_alg/yzl/Recognition/AFile/datasets/hmdb51", example['path'])
                },
                {
                    "type": "text",
