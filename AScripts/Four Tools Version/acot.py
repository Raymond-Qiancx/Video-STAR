...
parser.add_argument('--video-explanation-json', type=str, default="/mnt/xmap_nas_alg/yzl/Recognition_Github/AFile/sft_description_2.json", help='Path to video explanation JSON file')
...
video_explanation_json = args.video_explanation_json

# 加载视频解释数据
video_explanation_dict = {}
try:
    with open(video_explanation_json, "r", encoding="utf-8") as f:
        video_explanation_data = json.load(f)
        for item in video_explanation_data:
            video_explanation_dict[item["path"]] = item["analysis"]
    print(f"Loaded {len(video_explanation_dict)} video explanations")
...
for dataset_name in ['your_data_name']:
    ...
    OUTPUT_PATH = "/mnt/xmap_nas_alg/yzl/Recognition_Github/AFile/test_cot_four_tools.json"
    PROMPT_PATH = "/mnt/xmap_nas_alg/yzl/Recognition_Github/AFile/sft.json"
    ...
    FIRST_TURN_TEMPLATE = (
    """
    I will show you a video of human action. Your task is to determine which visual analysis tools would help you better identify the action. You have four independent tools available:
    ...
    3. **Video Explanation Tool**: Provides textual description of video content including body movements, interaction patterns, and environmental context
    4. **Noun Explanation Tool**: Provides detailed explanations of action types to help with classification
    ...
    3. **For Video Explanation**: Consider whether a textual description of the video content would help you better understand the action
    4. **For Noun Explanation**: Consider whether you need detailed explanations of action categories to make accurate classification
    ...
    [4] Video explanation evaluation: [assess whether textual description of video content would help]
    [5] Noun explanation evaluation: [assess whether action category details would help]
    [6] Final tool selection reasoning: [explain your decisions for each tool]
    </think>
    <pose_estimation>yes or no</pose_estimation>
    <person_detection>yes or no</person_detection>
    <video_explanation>yes or no</video_explanation>
    ...
    **Stage 2: Movement Pattern Refinement**
    ...
    def extract_first_turn_decisions(text):
        ...
        # 提取视频解释决策
        video_exp_pattern = r'<video_explanation>\s*(yes|no)\s*</video_explanation>'
        video_exp_match = re.search(video_exp_pattern, text, re.IGNORECASE)
        if video_exp_match:
            decisions['video_explanation'] = video_exp_match.group(1).lower()
    ...
    for i in tqdm(range(0, len(first_turn_messages), BSZ), desc="First turn - Tool decision"):
        ...
        try:
            ...
            print('Exception:', e)
            # 如果出错，默认不使用任何工具
            default_decisions = {'pose_estimation': 'no', 'person_detection': 'no', 'video_explanation': 'no', 'noun_explanation': 'no'}
    ...
    for i, x in enumerate(data):
        ...
        # 根据第一轮决策选择视频路径和答案格式
        decisions = first_turn_decisions[i] if i < len(first_turn_decisions) else {'pose_estimation': 'no', 'person_detection': 'no', 'video_explanation': 'no', 'noun_explanation': 'no'}

        # 确定是否需要名词解释
        need_noun_explanation = decisions['noun_explanation'] == 'yes'

        # 确定是否需要视频解释
        need_video_explanation = decisions['video_explanation'] == 'yes'
        ...
        # 构建视频解释文本
        if need_video_explanation and x['path'] in video_explanation_dict:
            video_explanation_text = f"""
    Video Explanation:
    Here is a textual description of the video content:
    {video_explanation_dict[x['path']]}
"""
        else:
            video_explanation_text = ""
    ...
    for i in tqdm(range(start_idx, len(messages), BSZ), desc="Processing batches"):
        ...
        for j, (sample, model_output) in enumerate(zip(data[i:i+BSZ], batch_output_text), start=i):
            ...
            # 添加第一轮决策信息
            sample_idx = i + j
            tool_decisions = first_turn_decisions[sample_idx] if sample_idx < len(first_turn_decisions) else {'pose_estimation': 'no', 'person_detection': 'no', 'video_explanation': 'no', 'noun_explanation': 'no'}
            ...
            sample["video_explanation"] = tool_decisions['video_explanation']
