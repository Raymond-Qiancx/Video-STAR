...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # 加载视频解释字典 (problem_id -> analysis)
        video_explanation_json_path = "/mnt/xmap_nas_alg/yzl/Recognition_Github/AFile/sft_description_2.json"
        self.video_explanation_dict = {}
        try:
            with open(video_explanation_json_path, 'r', encoding='utf-8') as f:
                video_explanation_data = json.load(f)
                for item in video_explanation_data:
                    self.video_explanation_dict[item["problem_id"]] = item["analysis"]
        except Exception as e:
            print(f"Warning: Could not load video explanation file: {e}")
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 处理两轮对话：先生成第一轮，再根据结果生成第二轮
        prompts = [x["prompt"] for x in inputs]
        
        # 第一轮对话：工具选择
        first_turn_prompts = [{"prompt": x["prompt"][:1]} for x in inputs]  # 只取第一轮对话，包装成字典格式
        first_turn_prompts_text = [maybe_apply_chat_template(example, self.processing_class)["prompt"] for example in first_turn_prompts]
        
        # 处理第一轮对话的视觉信息
        input_copy = copy.deepcopy(inputs[0]['prompt'][:1])  # 只取第一轮
        input_copy = self.remove_none_from_data(input_copy)
        
        if inputs[0]['data_type'] == 'image':
            input_copy[0]['content'][0]['image'] = "/mnt/xmap_nas_alg/yzl/Recognition/AFile/datasets/hmdb51/" + inputs[0]['path']
        elif inputs[0]['data_type'] == 'video':
            input_copy[0]['content'][0]['video'] = "/mnt/xmap_nas_alg/yzl/Recognition/AFile/datasets/hmdb51/" + inputs[0]['path']
            
        try:
            first_turn_image_inputs, first_turn_video_inputs, first_turn_video_kwargs = process_vision_info(input_copy, return_video_kwargs=True)
        except Exception as e:
            print(f"process_vision_info error for first turn, using fixed data, {e}")
            if inputs[0]['data_type'] == 'image':
                input_copy[0]['content'][0]['image'] = "/mnt/xmap_nas_alg/yzl/Recognition/AFile/datasets/hmdb51/" + 'Math/Multimath-300k/17ff4c7d14c388134de02381b1fc2824.png'
            elif inputs[0]['data_type'] == 'video':
                input_copy[0]['content'][0]['video'] = "/mnt/xmap_nas_alg/yzl/Recognition/AFile/datasets/hmdb51/" + 'LLaVA-Video-178K/liwei_youtube_videos/videos/youtube_video_2024/ytb_7nRmsEw7nsE.mp4'
            first_turn_image_inputs, first_turn_video_inputs, first_turn_video_kwargs = process_vision_info(input_copy, return_video_kwargs=True)
        
        # 处理第一轮对话
        first_turn_prompt_inputs = self.processing_class(
            text=copy.deepcopy(first_turn_prompts_text),
            images=first_turn_image_inputs,
            videos=first_turn_video_inputs,
            return_tensors="pt",
            padding=True,
            padding_side="left",
            add_special_tokens=False,
        )
        first_turn_prompt_inputs = super()._prepare_inputs(first_turn_prompt_inputs)
        
        # 生成第一轮回复（工具选择）
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            first_turn_completion_ids = unwrapped_model.generate(**first_turn_prompt_inputs, generation_config=self.generation_config)
            first_turn_prompt_length = first_turn_prompt_inputs["input_ids"].size(1)
            first_turn_completion_ids = first_turn_completion_ids[:, first_turn_prompt_length:]
        
        # 解码第一轮回复
        first_turn_completions = self.processing_class.batch_decode(first_turn_completion_ids, skip_special_tokens=True)
        ...
        def extract_tool_decisions(first_turn_output: str) -> dict:
            ...
            # 提取视频解释决策
            video_pattern = r'<video_explanation>\s*(yes|no)\s*</video_explanation>'
            video_match = re.search(video_pattern, first_turn_output, re.IGNORECASE)
            if video_match:
                decisions['video_explanation'] = video_match.group(1).lower()
        ...
        # 为每个样本提取工具决策并构建第二轮对话
        second_turn_prompts = []
        for i, (first_turn_completion, input_data) in enumerate(zip(first_turn_completions, inputs)):
            tool_decisions = extract_tool_decisions(first_turn_completion)
            
            # 根据工具决策选择视频路径
            video_path = get_video_path(input_data['path'], tool_decisions['pose_estimation'], tool_decisions['person_detection'])
            
            # 根据是否需要名词解释选择答案格式
            need_noun_explanation = tool_decisions['noun_explanation'] == 'yes'
            allowed_answers_str = get_allowed_answers_str(input_data.get('actions', {}), need_noun_explanation)
            
            # 根据是否需要视频解释选择文本
            need_video_explanation = tool_decisions['video_explanation'] == 'yes'
            video_analysis = self.video_explanation_dict.get(input_data.get('problem_id'), '')
            if need_video_explanation and video_analysis:
                video_explanation_text = f"""
    Video Explanation:
    Here is a textual description of the video content:
    {video_analysis}
"""
            else:
            ...
            # 构建注释视频路径
            annotated_video_path = os.path.join("/mnt/xmap_nas_alg/yzl/Recognition/AFile/datasets/hmdb51_pose", input_data['path'])
            
            # 构建第二轮对话 - 包含完整对话历史
            second_turn_template = input_data.get('second_turn_template', '')
            second_turn_text = second_turn_template.format(
                ALLOWED_ANSWERS=allowed_answers_str,
                ANNOTATED_VIDEO_PATH=annotated_video_path,
                VIDEO_EXPLANATION_TEXT=video_explanation_text
            )

            # 获取第一轮的用户消息（只保留文本内容，去除视频以避免重复处理）
            first_turn_user_message = input_data['prompt'][0].copy()
            # 移除视频内容，只保留文本，避免重复处理视频
            first_turn_user_message['content'] = [item for item in first_turn_user_message['content'] if item.get('type') == 'text']

            # 第一轮助手回复（纯文本）
            first_turn_assistant_message = {
                "role": "assistant",
                "content": first_turn_completion
            }
                ...
                "content": [
                    {
                        ...
                        input_data['data_type']: video_path
                    },
                    {
                        "type": "text",
            ...
            second_turn_prompts.append(full_conversation)
        
        # 现在处理第二轮对话
        second_turn_prompts_dict = [{"prompt": prompt} for prompt in second_turn_prompts]  # 包装成字典格式
        prompts_text = [maybe_apply_chat_template(example, self.processing_class)["prompt"] for example in second_turn_prompts_dict]

        # 处理第二轮对话的视觉信息
        # 提取包含视频的第二轮用户消息（对话中的最后一条消息）
        video_message = second_turn_prompts[0][-1]  # 最后一条消息包含视频
        input_copy = [video_message]  # 包装成消息列表格式
        ...
        try:
            ...
            print(f"process_vision_info error for second turn, using fixed data, {e}")
            if inputs[0]['data_type'] == 'image':
                input_copy[0]['content'][0]['image'] = "/mnt/xmap_nas_alg/yzl/Recognition/AFile/datasets/hmdb51/" + 'Math/Multimath-300k/17ff4c7d14c388134de02381b1fc2824.png'
            elif inputs[0]['data_type'] == 'video':
                input_copy[0]['content'][0]['video'] = "/mnt/xmap_nas_alg/yzl/Recognition/AFile/datasets/hmdb51/" + 'LLaVA-Video-178K/liwei_youtube_videos/videos/youtube_video_2024/ytb_7nRmsEw7nsE.mp4'
        ...
        if self.temporal and video_inputs:
            ...
            for i, (reward_func, reward_processing_class) in enumerate(
                ...
                # 传入第一轮输出（工具选择）用于奖励中的工具使用加分
                shuffled_reward_kwargs["first_turn_outputs"] = []
                for _idx in range(len(inputs)):
                    shuffled_reward_kwargs["first_turn_outputs"].extend([first_turn_completions[_idx]] * self.shuffled_num_generations)
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            # 传入第一轮输出（工具选择）用于奖励中的工具使用加分
            reward_kwargs["first_turn_outputs"] = []
            for _idx in range(len(inputs)):
                reward_kwargs["first_turn_outputs"].extend([first_turn_completions[_idx]] * self.num_generations)
        ...
        # print("test0: ", rewards)
        # print("test1: ", completion_mask.sum(1))
